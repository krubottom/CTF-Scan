...
def nmap_scan(ip_address, output_directory, dns_server, quick):
   ...
   if dns_server:
       ...
       UDPSCAN = "nmap -vv -Pn -A -sC -sU -T 4 --top-ports 200 --max-retries 0  -oN '%s/%sU.nmap' -oX '%s/%sU_nmap_scan_import.xml' %s" % (output_directory, ip_address, output_directory, ip_address, ip_address)
